...
Cron.schedule('0 0 0 * * *', () => {
  settings.GetSubGuilds().then(array => {
    ...
    array.forEach((item, i) => {
      ...
      game.CanStartGame(item.id).then(() => {
...
DiscordClient.on('guildCreate', guild => {
  ...
  const helloEmbed = new DiscordJS.MessageEmbed()
    .setColor('#FF6699')
    .setTitle('Привет!')
    .setDescription(`Спасибо за спользование бота!`)
...
DiscordClient.on('message', msg => {
  ...
  if (msg.content.match(/^!участвовать/) || msg.content.match(/^!участв/)) {
  ...
  if (msg.content.match(/^!кто/)) {
    game.CanStartGame(msg.guild.id).then(() => {
      ...
      msg.channel.send("А победитель сегодня - " + reject);
    });

    ChatFunctions.deleteMessage(msg, 1000);
  }

  if (msg.content.match(/^!топпобдителей/) || msg.content.match(/^!топ/)) {
  ...
  if (msg.content.match(/^!п команды/) || msg.content.match(/^!п помощь/)) {
    ...
    const helpEmbed = new DiscordJS.MessageEmbed()
      ...
      Список команд доступных для бота:`)
	    .addFields(
		      { name: '!участвовать', value: '- участвовать в игре' },
          { name: '!кто', value: '- запустить игру (или узнать кто уже был сегодня выбран)' },
          { name: '!топ', value: '- вывести топ 10 на этом сервере' },
  ...
  if (msg.content.match(/^!п подписка/)) {
    ...
    const subEmbed = new DiscordJS.MessageEmbed()
      ...
      Преимущества подписки:`)
	    .addFields(
		      { name: 'Автосообщения', value: 'Автоматически в 00:00 по МСК запускает !кто . Теперь вам не придётся жалеть, если вы пропустили день и забыли прописать команду! Вы сможете выбрать на какой канал бот будет писать эти автосообщения' },
	    )
	    .setFooter("Автор: Гусик#9344", myAvatarURL );
